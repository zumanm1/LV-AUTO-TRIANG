...
Configuration commands:`;
  ...
  const handleValidateCommands = async () => {
    ...
    try {
      // Use AI to validate each step
      for (let i = 0; i < steps.length; i++) {
        const step = steps[i];
        const validationPrompt = `You are an AI Network Engineering Assistant validating Cisco IOS configuration commands.
...
Please analyze the configuration for this specific validation step. Respond with either "VALID" or "INVALID" followed by a brief explanation.
        ...
        setAutomationSteps((prev) =>
          prev.map((s, index) => {
            if (index === i) {
              return {
                ...s,
                status: validationResult.toLowerCase().includes("valid") ? "completed" : "failed",
